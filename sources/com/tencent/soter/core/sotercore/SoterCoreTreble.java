...
public class SoterCoreTreble extends SoterCoreBase implements ConstantsSoter, SoterErrCode {
    ...
    private IBinder.DeathRecipient mDeathRecipient = new IBinder.DeathRecipient() { // from class: com.tencent.soter.core.sotercore.SoterCoreTreble.1
        ...
        public void binderDied() {
            ...
            if (SoterCoreTreble.mSoterService == null || SoterCoreTreble.mSoterService.asBinder() == null) {
                return;
            }
    ...
    private ServiceConnection mServiceConnection = new ServiceConnection() { // from class: com.tencent.soter.core.sotercore.SoterCoreTreble.2
        ...
        public void onServiceConnected(ComponentName componentName, IBinder iBinder) throws RemoteException {
            ...
            try {
                ...
                SLogger.e(SoterCoreTreble.TAG, "soter: Binding deathRecipient is error - RemoteException" + e.toString(), new Object[0]);
            }
            if (SoterCoreTreble.this.serviceListener != null) {
    ...
    public static long getFib(long j) {
        long j2 = 0;
        if (j < 0) {
            return -1L;
        }
    ...
    @Override // com.tencent.soter.core.sotercore.SoterCoreBase
    public Signature initAuthKeySignature(String str) throws NoSuchAlgorithmException, InvalidKeyException, IOException, KeyStoreException, CertificateException, NoSuchProviderException, UnrecoverableEntryException {
        return null;
    }
    ...
    public void rebindService() {
        if (this.canRetry) {
            ...
            long jElapsedRealtime = (SystemClock.elapsedRealtime() - this.lastBindTime) / 1000;
            long fib = getFib(this.disconnectCount);
            long j = fib - jElapsedRealtime;
            SLogger.i(TAG, "fib: %s, rebind delay: %sS", Long.valueOf(fib), Long.valueOf(j));
            if (j <= 0) {
                bindService();
            } else {
    ...
    @Override // com.tencent.soter.core.sotercore.SoterCoreBase
    public boolean initSoter(Context context) throws InterruptedException {
    ...
    public void bindService(boolean z) {
        ...
        scheduleTimeoutTask(z);
    }

    private void scheduleTimeoutTask(boolean z) {
        long fib = getFib(this.noResponseCount);
        SLogger.i(TAG, "soter: scheduleTimeoutTask isCycle:%b noResponseCount:%d checkDelay:%d ", Boolean.valueOf(z), Integer.valueOf(this.noResponseCount), Long.valueOf(fib));
        if (z || this.noResponseCount <= 3) {
    ...
    public SoterPubKeyModel getAppGlobalSecureKeyModel() {
        ...
        try {
            ...
            if (bArr != null && bArr.length > 0) {
                return retrieveJsonFromExportedData(bArr);
            }
    ...
    public byte[] finishSign(long j) throws Exception {
        ...
        try {
            SoterSignResult soterSignResultFinishSign = mSoterService.finishSign(j);
            bArr = soterSignResultFinishSign.exportData;
            if (soterSignResultFinishSign.resultCode == 0) {
                return bArr;
            }
            ...
            new Thread(new Runnable() { // from class: com.tencent.soter.core.sotercore.SoterCoreTreble.6
                ...
                public void run() {
                    try {
                        ...
                        SoterExtraParam extraParam = SoterCoreTreble.mSoterService.getExtraParam(ISoterExParameters.FINGERPRINT_TYPE);
                        if (extraParam != null && (extraParam.result instanceof Integer)) {
                        ...
                        if (extraParam2 == null || !(extraParam2.result instanceof Integer[])) {
                            return;
                        }
                        SoterExParametersTrebleImpl.setParam(ISoterExParameters.FINGERPRINT_HARDWARE_POSITION, extraParam2.result);
                    } catch (Exception e) {
                        SLogger.printErrStackTrace(SoterCoreTreble.TAG, e, "soter: getExtraParam fail");
                    }
                }
            }).start();
        } catch (Exception e) {
